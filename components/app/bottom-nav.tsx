...
export function BottomNav() {
  ...
  const navItems = [
    ...
    {
      href: "/market-intelligence?view=settings",
      label: t.navSettings,
      icon: Settings,
      view: "settings",
    },
  ...
  return (
    <nav
      className="mobile-bottom-nav fixed inset-x-0 bottom-[max(0.5rem,env(safe-area-inset-bottom))] z-50 mx-auto w-[min(calc(100%_-_1rem),27rem)] rounded-[1.35rem] border border-white/10 bg-[#08131e]/92 px-1.5 pt-1.5 shadow-[0_18px_55px_rgba(0,0,0,0.42)] backdrop-blur-2xl xl:hidden"
        ...
        {navItems.map(({ href, label, icon: Icon, view }) => {
          ...
          return (
            <Link
              ...
              className={cn(
                "app-touch relative flex min-h-[3.2rem] min-w-0 flex-1 flex-col items-center justify-center gap-1 rounded-[1rem] px-1.5 py-1.5 text-[10px] font-semibold transition-all duration-200",
                active
